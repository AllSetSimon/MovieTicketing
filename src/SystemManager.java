...
public class SystemManager {
	...
	public SystemManager() {
		...
		// 고객 목록 추가
		customerList.add(new Customer("갈아만든배", "1", "강구현", 95000));
		customerList.add(new Customer("Simon", "2", "박수빈", 55000));
		customerList.add(new Customer("뜬혁", "3", "신승혁", 43000));
		customerList.add(new Customer("SkyWalker", "4", "임준석", 57000));
		customerList.add(new Customer("svra0945", "5", "장동주", 50000));
	...
	public int showDetail(int selectNum) { // 영화 상세정보 출력 메서드
		...
		System.out.println("관객평점 : " + String.format("%.2f", showingList.get(selectNum - 1).getRating()) + "점");
