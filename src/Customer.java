...
public class Customer {
	...
	public void infarmaion() {
		Customer list = new Customer();
		Movie movie = new Movie();
		list.rsvList.add(new ReserveList(movie.getTitle(), null, null, 0, null, 0)); //0
		...
		for (int i = 0; i < list.rsvList.size(); i++) {
			...
			System.out.println("4.상영관 및 시간:"+list.rsvList.get(i).getShowRoomNum()+list.rsvList.get(i).getStartTime());
			System.out.println("5.좌석번호:"+list.rsvList.get(i).getSeatNum());
			System.out.println("6.가격:");
		}		
		/* 향상
		for (ReserveList reserveList : list.rsvList) {			
			System.out.println(reserveList.getTitle());
		...
		*/
		System.out.println("==================================");	
		System.out.println("예매를 원하시면 ‘Y’ 처음으로 돌아가려면 ‘N’을 입력하세요 ");	
		Scanner scan = new Scanner(System.in);		
		String ticketing = scan.nextLine();				
